...
| ID | Task | Why | 공수 | 파일 |
|----|------|-----|------|------|
| T0-1 | **Paddle Sandbox 계정 설정 + 통합 테스트** | 결제 못 받으면 무료 서비스 | 8h | `lib/paddle/` |
| T0-2 | **Paddle Production 키 발급 + 배포** | Sandbox 테스트 후 즉시 전환 | 2h | `.env.production` |
...
| ID | Task | Why | 공수 | 파일 |
|----|------|-----|------|------|
| T1-1 | **분석 완료 이메일 (E-04)** | 창 닫으면 결과 모름 → 재방문 0% | 4h | `app/api/webhooks/worker/` |
| T1-2 | **분석 실패 이메일 (E-05)** | 환불됐는데 모르면 CS 폭발 | 2h | 동일 |
| T1-3 | **크레딧 부족 경고 이메일 (E-07, E-08)** | Pro 전환 트리거 | 3h | `app/api/upload/` |
| T1-4 | **Sentry DSN 설정 + Slack 알림** | 장애 인지 불가 = 서비스 사망 | 4h | `next.config.ts`, Worker |
...
| ID | Task | Why | 공수 | 파일 |
|----|------|-----|------|------|
| T2-1 | **온보딩 Step 1-2 (환영 + 첫 업로드)** | 뭘 해야 할지 모르면 이탈 | 4h | `app/(dashboard)/` |
| T2-2 | **온보딩 Step 3-5 (검색/검토/내보내기)** | 핵심 기능 체험 유도 | 4h | 동일 |
| T2-3 | **환영 이메일 (E-01)** | 브랜드 첫인상 + 온보딩 링크 | 2h | `lib/email/` |
| T2-4 | **크레딧 리셋 Cron 백업** | 리셋 안 되면 무료 사용자 CS 폭발 | 4h | `app/api/cron/credit-reset/` |
...
| ID | Task | Why | 공수 | 파일 |
|----|------|-----|------|------|
| T3-1 | **LLM 재시도 로직 적용** | Timeout 시 크레딧만 날아감 | 4h | `apps/worker/services/llm_manager.py` |
| T3-2 | **메트릭 정확도 개선** | 비용 추정 틀리면 마진 계산 불가 | 4h | `llm_manager.py`, `metrics_service.py` |
| T3-3 | **결제 이메일 (E-10, E-11, E-12)** | 결제 실패/시작/취소 알림 | 4h | `app/api/webhooks/paddle/` |
| T3-4 | **E2E CI/CD 연동** | 배포할 때마다 수동 테스트 지옥 | 4h | `.github/workflows/` |
...
### ⚪ TIER 4: 데이터 기반 개선 (Beta 피드백 후)

> **유저 불만이 쌓이면 그때 한다**

| ID | Task | 트리거 조건 | 우선순위 |
|----|------|------------|----------|
| T4-1 | strict schema 적용 | 분석 결과 불일치 불만 5건+ | 피드백 후 |
| T4-2 | CoT/Few-shot 프롬프트 | 정확도 불만 10건+ | 피드백 후 |
| T4-3 | CoverageCalculator + GapFiller | 필드 누락 불만 10건+ | 피드백 후 |
| T4-4 | ResumeIntentGuard (문서 분류) | 비이력서 업로드 비율 >5% | 데이터 기반 |
| T4-5 | 3-Way Cross-Check | Pro 유저 10명+ 확보 후 | Pro 전용 |
