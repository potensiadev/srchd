...
**결론: 기술적으로는 95% 완료, "비즈니스 운영" 관점에서는 65% 상태** (Sentry 연동 완료)
...
| 순위 | ID | Task | Why | 공수 | 파일 | 상태 |
|------|-----|------|-----|------|------|------|
| **3** | T1-1 | **분석 완료 이메일 (E-04)** | 창 닫으면 결과 모름 → 재방문 0% | 4h | `app/api/webhooks/worker/` | 🔜 대기 |
| ~~**4**~~ | ~~T1-4~~ | ~~**Sentry DSN 설정**~~ | ~~장애 인지 불가 = 서비스 사망~~ | ~~2h~~ | ~~`next.config.ts`, Worker~~ | ✅ 완료 |
| **5** | T1-5 | **Sentry Slack 알림 연동** | 에러 실시간 알림 | 1h | Sentry Dashboard | 🔜 대기 |
| **6** | T1-2 | **분석 실패 이메일 (E-05)** | 환불됐는데 모르면 CS 폭발 | 2h | 동일 | 🔜 대기 |
| **7** | T1-3 | **크레딧 부족 경고 이메일 (E-07, E-08)** | Pro 전환 트리거 | 3h | `app/api/upload/` | 🔜 대기 |
...
| ~~**9**~~ | ~~T2-4~~ | ~~**크레딧 리셋 Cron 백업**~~ | ~~리셋 안 되면 무료 사용자 CS 폭발~~ | ~~4h~~ | ✅ **완료** |
...
[x] Sentry DSN 설정 완료 (Frontend + Worker)
